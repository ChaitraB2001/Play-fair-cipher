...
void main()
{
	...
	printf("\n plain text afteer space remove:%s ",msg);
...
char encryption(char pt[])
{
	...
	for(i=0;i<strlen(pt);){
		...
		if(r1==r2){ct[i]=play[r1][(c1+1)%5];ct[j]=play[r2][(c2+1)%5];putchar(ct[i]);putchar(ct[j]);
		}else if(c1==c2){ct[i]=play[(r1+1)%5][c1];ct[j]=play[(r2+1)%5][c2];putchar(ct[i]);putchar(ct[j]);
		}else{ct[i]=play[r1][c2];ct[j]=play[r2][c1];putchar(ct[i]);putchar(ct[j]);
		}i=i+2;j=j+2;
	}
	printf("\n\n\t Encrypted msg is %s\n",ct);
	return ct;
